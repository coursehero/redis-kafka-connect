...
class StreamMessageRecovery {
	...
	StreamMessageRecovery(GenericObjectPool<StatefulConnection<String, String>> pool, Consumer<String> consumer,
			RedisSourceConfig config, AckPolicy ackPolicy, String lastCommitted) {
	...
	List<StreamMessage<String, String>> recoverMessages() throws Exception {
		...
		try (final StatefulConnection<String, String> connection = pool.borrowObject()) {
			...
			while (unprocessed.size() < batchSize) {
				...
				// and doesn't block.
				final List<StreamMessage<String, String>> messages = commands.xreadgroup(consumer,
				...
				needsAck.stream().collect(groupingBy(StreamMessage::getStream))
						.forEach((stream, msgs) -> commands.xack(stream, config.getStreamConsumerGroup(),
							msgs.stream().map(StreamMessage::getId).peek(x -> {
								System.out.println("XACK " + x);
